...
public class PrimaryController {
    ...
    /**
     * Shows the user an output of their input to ensure it is the number of
     * submissions they wish to import. After displaying the user's input, it
     * will show a brief instruction on how to proceed.
     */
    ...
    public void handleFileOpenMenuItem() throws FileNotFoundException {
        ...
        if (selectedFile != null) {
            ...
            //try-catch code here
            //SETS HIGHEST SCORE OF SUBMISSION
            double[] scores = {1};
            double highestScore = scores[0];
            
            for (int i = 1; i < sa.length; i++) {
                if (sa[i].getScore() > highestScore) {
                    highestScore = sa[i].getScore();
                    textFieldMessage.setText(String.valueOf(sa[i].getScore()));
                }
            }
            ...
            for (int e = 0; e < sa.length; e++) {
                submissionsList.add(sa[e]);
                /*+ ", " + submission[e].getScore() + ", " + submission[e].getDate().getMonth() + "/"
    ...
    public void handleListViewItemsMouseClick(MouseEvent event) {
        ...
        if (selectedIndex >= 0 && selectedIndex < submissionsList.size()) {
            ...
            //Submission e;
            sa = submissionsList[selectedIndex];

            textFieldAssignment.setText(sa.getAssignment());
    ...
    @FXML
    /**
     * Secondary method that switches from Primary Controller class to the
     * Secondary Controller class.
     */
